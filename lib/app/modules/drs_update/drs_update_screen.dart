...
class DrsUpdateScreen extends GetView<DrsUpdateController> {
  const DrsUpdateScreen({super.key});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('DRS Update', style: TextStyle(color: Colors.white)),
      ...
      body: Obx(() {
          ...
          return Center(
            child: Column(
              ...
              children: [
                ...
                const Text(
                  "No pending DRS found",
                  style: TextStyle(fontSize: 16, color: Colors.grey, fontWeight: FontWeight.w500),
                ),
                const SizedBox(height: 24),
                ElevatedButton.icon(
                  onPressed: () => controller.fetchDrsList(),
        ...
        return RefreshIndicator(
          onRefresh: () => controller.fetchDrsList(),
          child: ListView.builder(
            padding: const EdgeInsets.all(16),
            itemCount: controller.drsList.length,
            itemBuilder: (context, index) {
              final drs = controller.drsList[index];

              String displayDate = drs['pdcdt'] ?? '';
              try {
                if (drs['pdcdt'] != null) {
                  DateTime dt = DateTime.parse(drs['pdcdt']);
